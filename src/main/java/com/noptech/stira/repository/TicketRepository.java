...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    @Query("SELECT t from Ticket t where t.stormStatus <> t.jiraStatus") List<Ticket> findWithDifferentStatus();

    @Query("SELECT t from Ticket t where t.stormKey is not null and t.stormStatus = 'WAITING_SSE'" +
        " and (t.jiraStatus <> 'WAITING_SSE' or t.jiraStatus is null)") List<Ticket> findPendingSSEWithDifferentStatus();
}
